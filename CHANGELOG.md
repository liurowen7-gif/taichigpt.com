...
- 邀请函 Tab（`.invitation-scope`）由米色素雅纸质风整体改为中式红金节庆风格
- 头部改为大红底 + 米金卷轴（上下金色轴杆 + 回纹角框），承载红色主标题与副题
- 新增顶/底金色祥云带（inline SVG）；视觉中心为红底金色双线方框太极卡，米白太极图配金色祥云环绕
- 信息区改为米金纸面 + 深红/暖金字，正文对比度达标；订阅按钮改红金
- 文案统一：zh「拜师礼」→「拜师仪式」（EN 保持 Ceremony）
- 仅作用于邀请函 Tab，预告 Tab 保持暗色金；作用域隔离
...
- 第 1 步简化为「向混元太极祖师冯志强先生上香」
- 第 3 步子项调整：叩拜礼提前至宣读拜师帖之后、**按年龄顺序**依次行礼；「师父回礼」单列
- 第 4 步改为「播放各地混元门人祝贺视频」，子项去掉「老弟子代表讲话」
- 原第 5 步「各地拳友分别展示」移除；合影环节文案简化
- 中英文同步更新
...
- Tab 栏原用写死像素（`top: 53px`）对齐 nav 底边，nav 实际高度随字体渲染浮动，交界处露出 2-3px 页面背景形成白线（手机端可见）
- 改为 JS 动态测量 nav 高度对齐并重叠 1px，桌面/移动端均不再依赖写死偏移
...
- **邀请函页** `/ceremony/invitation.html`：可直接分享的静态邀请函（米色纸质请柬风，参考线下邀请函版式）
  - 结构：山水页头（太极图+主题句"太极传薪火，四海共传承"）→ 邀请正文 → 大字日期 07·25 → 地点/着装要求 → 仪式流程 6 项 → 邮件订阅
  - 双语（EN/中文），与全站共享语言偏好；OG 标签支持微信/WhatsApp 分享预览
  - 订阅表单直连既有 Cloudflare Worker（Lark Bitable 存档）
- **仪式页入口**：Hero 区新增金色实心按钮"查看邀请函 →"（置于"敬请期待"之上）
- Sitemap 新增 invitation URL
- ⚠️ 待补充：具体时间（页面暂显示"以正式通知为准"）、流程如需按实际调整
...
- **仪式预告页** `/ceremony/`：拜师仪式介绍 + "敬请期待"，仪式后填充照片与全记录
- 页面含：仪式含义（拜师/传承之约）、日期地点、订阅 CTA，双语覆盖，与主站共享语言偏好（localStorage）
- **首页入口 ×2**：Hero 标题上方金色公告条（Jul 25 · Singapore — Discipleship Ceremony →）+ 导航栏金色 "Ceremony / 拜师仪式" 链接（置于首位）
- Sitemap 新增 /ceremony/ URL

**手机端修复（P0）**

- **修复横向溢出**：Stats 数据条固定 4 列 grid 在小屏撑破页面（390px 视口被撑到 498px，全页错位）；≤768px 降为 2 列 + 允许收缩
- **新增汉堡菜单**：此前 ≤768px 导航链接直接隐藏、无任何菜单，手机用户无法到达文化/博客/课程板块；现为全站导航下拉面板，拜师仪式入口带 "7月25日" 标签
- **Hero 视频**：补 `muted` + `preload="metadata"` 属性，修复 iOS Safari 自动播放不稳定；有声播放仍由声音开关控制
